...
public interface BoardService {
    ...
    // 장소 포스팅 정보 조회
    BoardInfoResponseDTO getBoardByBno(Long bno) throws IllegalAccessException, SQLException;

    // 코스 포스팅 정보 조회
    BoardInfoResponseDTO getCourseByBno(Long bno) throws IllegalAccessException, SQLException;

    // 메인 포스팅 조회
    MainResponseDTO mainBoard(Long mno);

    // 회원별 장소 포스팅 정보 조회, 실패시 null 반환
    List<BoardMemberResponseDTO> getBoardByMno(Long mno);

    // 장소별 장소 포스팅 정보 조회
    List<BoardByPnoResponseDTO> getBoardByPno(Long pno) throws IllegalAccessException, SQLException;

    // 회원별 코스 포스팅 정보 조회
    List<BoardMemberResponseDTO> getCourseBoardByMno(Long mno);

    // 코스 검색 조회
    List<BoardSearchResponseDTO> findCourseBoard(String search);
